...
if ( ! function_exists( 'orbital_dequeue_styles' ) ) :
	...
	function orbital_dequeue_styles() {
		...
		if(is_admin() || is_customize_preview() || ! orbital_customize_option('orbital_performance_render_blocking_css')) return;

		global $wp_styles;
		wp_dequeue_style( 'thickbox.css' );

		$front_css = $wp_styles->queue;
		$front_css = array_diff($front_css, ['admin-bar', 'yoast-seo-adminbar']);
		$content_style = '';

		foreach( $wp_styles->registered as $asset ) :

			if(in_array($asset->handle, $front_css)){
				if(current_user_can('administrator') && ! is_wp_error(wp_remote_get($asset->src))) {
					$buffer = wp_remote_get($asset->src)['body'];
...
if ( ! function_exists( 'orbital_dequeue_scripts' ) ) :
	...
	function orbital_dequeue_scripts(){
		if(is_admin() || is_customize_preview() || ! orbital_customize_option('orbital_performance_render_blocking_js')) return;
